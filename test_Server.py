...
class TestServer(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.dist['st'] = Distribution("scipy.stats.expon(scale=300)")
        ...
        self.assertEqual(100, self.server.availableSince)
        self.assertTrue(isinstance(self.server.__str__(), str))
        self.assertTrue(isinstance(self.server.__repr__(), str))
    ...
    def test_downTimeDistribution(self):
        ...
        np.random.seed(200)
        self.server.downTimeDistribution = Distribution("scipy.stats.norm(100,30)")
        actual = self.server._downTimeDistribution.getEvent()
        self.assertAlmostEqual(expected, actual)
    ...
    def test_oosDistribution(self):
        ...
        np.random.seed(200)
        self.server.oosDistribution = Distribution("scipy.stats.norm(100,30)")
        actual = self.server._oosDistribution.getEvent()
        self.assertAlmostEqual(expected, actual)
    ...
    def test_serviceTimeDistribution(self):
        ...
        np.random.seed(200)
        self.server.serviceTimeDistribution = Distribution("scipy.stats.norm(100,30)")
        actual = self.server._serviceTimeDistribution.getEvent()
        self.assertAlmostEqual(expected, actual)
    ...
    def test_processEvent(self):
        ...
        # verify resume service doesn't work when busy
        self.assertFalse(self.server.resumeService(10**6))
        ...
        # verify resume service doesn't work when available
        self.assertFalse(self.server.resumeService(10**6))
        ...
        # verify that pauseService does not work when server is OOS
        self.assertFalse(self.server.pauseService(100))
        ...
        # verify resume service doesn't work when PENDING_OOS
        self.assertFalse(self.server.resumeService(10**6))

        # verify that we cannot pause service when alreading pending
        self.assertFalse(self.server.pauseService(100))
...
#     def setUp(self) -> None:
#         self.dist = {}
#         self.dist['dt'] = Distribution("scipy.stats.norm(100,50)")
#         self.dist['oos'] = Distribution("scipy.stats.norm(100,50)")
#         self.dist['st'] = Distribution("scipy.stats.expon(300)")
#
#         # uncomment to test validity of server construction
#         # for k, v in self.dist.items():
#         #     with self.subTest(dist=k):
...
#         for k, v in self.dist.items():
#             with self.subTest(dist=k):
#                 self.assertTrue(v.isValid(v.RNG))
#
#         self.dist['invalid'] = Customer('teddy', 0)
#
#         self.server = Server('S1', 100,
...
#         test._nextDownTime = 2000
#         test._nextEventTime = 10000
#         self.assertEqual(ServerState.PENDING_OOS, test.status)
#
#         # state 3 OOS
#         # nextEventTime
#         test._nextEventTime = 12000
...
#         test._nextEventTime = 10000
#         #
#         self.assertEqual(ServerState.INVALID, test.status)
#
#         # state 5
#         test._nextEventType = ServerEvent.SERVICE_COMPLETION
#         self.assertEqual(ServerState.INVALID, test.status)
...
#         self.server.processEvent(expSvcCompTime)
#         self.assertEqual(ServerState.AVAILABLE, self.server.status)
#         self.assertAlmostEqual(self.server._nextDownTime, self.server.nextEventTime)
#
#         # verify resume service doesn't work when available
#         self.assertFalse(self.server.resumeService(10**6))
#
