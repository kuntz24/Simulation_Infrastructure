...
class TestSimQueue(TestCase):
    ...
    def setUp(self) -> None:
        ...
        # then setup servers
        self.dist = {}
        self.dist['dt'] = Distribution("scipy.stats.norm(16400,2000)")
        self.dist['oos'] = Distribution("scipy.stats.norm(700,400)")
        self.dist['st'] = Distribution("scipy.stats.expon(1/300)")
    ...
    def test_processEvent1(self):
        ...
        # next event should be a server down on server 1, so let's get the return
        # to service time to process the events
        rstate = np.random.get_state()
        expUpTime = self.servers[0]._nextDownTime + self.servers[0]._oosDistribution.getEvent()
        np.random.set_state(rstate)
        ...
        self.assertEqual(ServerState.OOS, self.servers[0].status)
